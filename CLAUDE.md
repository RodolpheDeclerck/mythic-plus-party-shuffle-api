...
New code must pass build and lint before any `git add`.
Tests are written manually by the developer after the PR is merged — do not run or generate tests.
...
### Unit tests — written manually by the developer

Unit tests are NOT generated by Claude Code. They are written by the developer in Cursor after the PR is merged.

Claude Code must NOT attempt to generate unit tests.
...
Claude Code must NOT attempt to generate integration tests.

**Why:** both test types require understanding the full system behavior, edge cases, and business rules that Claude Code cannot reliably produce within its turn budget.

**Convention for manually written tests:**
- Unit tests: `*.test.ts` next to the file under test
- Integration tests: `tests/integration/<bounded-context>.integration.test.ts`
